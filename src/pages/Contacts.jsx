  ...
  useEffect(() => {
    ...
    if (authData) {
      ...
      const firstProjectId =
        parsedAuthData.claims.project &&
        parsedAuthData.claims.project.length > 0
          ? parsedAuthData.claims.project[0]
          : null;

      if (firstProjectId) {
        axios
          .get(`${config.backendUrl}/api/projects/${firstProjectId}`, {
            headers: {
        ...
        {data &&
          ...
          data.members.map((member) => (
            ...
            <div key={member.ID} className="member">
              ...
              <div className="texts">
                <h2>
                  Имя: {member.User.FirstName} {member.User.LastName}
                </h2>
                <h2>Должность: {member.Role.name}</h2>
                <h2>Номер телефона: {member.User.PhoneNumber}</h2>
              </div>
            </div>
          ))}
      </div>
