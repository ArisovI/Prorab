...
import styles from "./AddPhotoFolder.module.scss";
...
export const AddPhotoFolder = () => {
  ...
  const createFolder = () => {
    ...
    axios
      .post(`${config.backendUrl}/api/photo-folder`, folder, {
        headers: {
  ...
  useEffect(() => {
    if (photoProjectID) {
      axios
        .get(
          `${config.backendUrl}/api/photo-folder/project/${photoProjectID}`,
          {
            headers: {
              Authorization: `Bearer ${userInfo.token}`,
            },
  ...
  return (
    <div>
      <div className={styles.managerPhotopage}>
        <div className={styles["managerPhotopage-container"]}>
