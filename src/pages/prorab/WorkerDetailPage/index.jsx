...
import styles from "./WorkerDetailPage.module.scss";
...
export const WorkerDetailPage = () => {
    ...
    if (userInfo) {
      try {
        const response = await axios.get(
          `${config.backendUrl}/api/details/project/${selectedProjectId}`,
          {
            headers: {
  ...
  return (
    <div className={styles["worker-detail"]}>
      <div className={styles["workerDetail-container"]}>
        {error && <div className={styles["error-message"]}>{error}</div>}{" "}
        {/* Error message display */}
        <label className={styles["workerDetail-label"]} htmlFor="">
          Выберите проект
        </label>
        <select
          className={styles["workerDetail-select"]}
          defaultValue={""}
        ...
        {projectDetails && (
          <form onSubmit={handleSubmit}>
            <h2 className={styles["workerDetail-header"]}>Детали проекта</h2>
            <div>
              <label className={styles["workerDetail-label"]}>
                Оплаченные материалы:
              </label>
              <input
                className={styles["workerDetail-input"]}
                type="number"
                name="paid_materials"
            ...
            <div>
              ...
              <input
                ...
                value={
                  projectDetails.project_price || projectDetails.price || ""
                }
                onChange={handleInputChange}
              />
            </div>

            <button type="submit" className={styles["workerDetail-button"]}>
              Сохранить изменения
            </button>
          </form>
        )}
