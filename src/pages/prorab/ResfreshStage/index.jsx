...
import styles from "./RefreshState.module.scss";
...
export const RefreshState = () => {
  ...
  const handleChangeProject = (e) => {
    ...
    if (userInfo) {
      axios
        .get(`${config.backendUrl}/api/states/${projectID}`, {
          headers: {
  ...
  const handleSave = (stade) => {
    const data = {
      ...
      necessary_money: parseInt(
        editStade[stade.ID]?.necessary_money ?? stade.necessary_money
      ),
    ...
    axios
      .patch(`${config.backendUrl}/api/states/${stade.ID}`, data, {
        headers: {
          Authorization: `Bearer ${userInfo.token}`,
  ...
  const renderStades = () => {
    ...
    return stades.map((stade) => (
      <div key={stade.ID} className={styles["refresh-stage"]}>
        <h3>Стадия: {stade.job}</h3>
        <p>
          <strong>ID стадии:</strong> {stade.ID}
        </p>
  ...
  return (
    <div className={styles["refresh-stage-first-container"]}>
      ...
      {stades && stades.length > 0 && (
        <div className={styles["refresh-stage-container"]}>
          {renderStades()}
        </div>
      )}
    </div>
